...
$search_params = apply_filters('avf_frontend_search_form_param', array(
	...
	'form_action' => get_permalink(get_page_by_title('Search')),
	'ajax_disable'	=> false
));
...
<form action="<?php echo $search_params['form_action']; ?>" id="searchform" class="<?php echo $disable_ajax; ?>">
	<div>
		<input type="submit" value="<?php echo $icon; ?>" id="searchsubmit" class="button <?php echo $class; ?>" />
		<input type="text" name="q" id="q" name="<?php echo $search_params['search_id']; ?>" value="<?php if (!empty($_GET['q'])) echo get_search_query(); ?>" placeholder='<?php echo $search_params['placeholder']; ?>' />
		<?php
